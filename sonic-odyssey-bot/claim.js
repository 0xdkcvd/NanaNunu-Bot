...
async function processPrivateKey(privateKey) {
  try {
    ...
    if (profile.wallet_balance > 0) {
      ...
      // Automatically open mystery boxes
      console.log(
        `[ ${moment().format('HH:mm:ss')} ] Please wait to open boxes...`.yellow
      );

      // Opening all available mystery boxes
      ...
      // Automatically perform daily login
      console.log(`[ ${moment().format('HH:mm:ss')} ] Please wait for daily login...`.yellow);
      const claimLogin = await dailyLogin(token, getKeypair(privateKey));
      if (claimLogin) {
        console.log(
          `[ ${moment().format('HH:mm:ss')} ] Daily login completed! Status: ${
            claimLogin.status
          } | Accumulative Days: ${claimLogin.data.accumulative_days}`.cyan
        );
      }
      console.log(
        `[ ${moment().format('HH:mm:ss')} ] All automated tasks completed for ${publicKey}!`.green
      );
    } else {
      console.log(
  ...
  try {
    ...
    for (let i = 0; i < PRIVATE_KEYS.length; i++) {
      ...
      if (i < PRIVATE_KEYS.length - 1) {
        const continueNext = true; // In production, this would typically be set by user interaction or condition
        if (!continueNext) break;
      }
    }
    console.log(`[ ${moment().format('YYYY-MM-DD HH:mm:ss')} ] All private keys processed.`.cyan);
  } catch (error) {
    console.log(`Error in bot operation: ${error}`.red);
  } finally {
    const nextRunTime = moment().add(24, 'hours').format('YYYY-MM-DD HH:mm:ss');
    console.log(
      `Next run scheduled at ${nextRunTime}. `.white
    );
  }
}
...
// Function to set interval for every 24 hours (86400000 milliseconds)
setInterval(() => {
  console.log(`[ ${moment().format('YYYY-MM-DD HH:mm:ss')} ] Starting bot again after 24 hours.`)
  startBot();
}, 86400000);

// Log initial scheduled run time
console.log(`Initial run scheduled at ${moment().format('YYYY-MM-DD HH:mm:ss')}.`);
