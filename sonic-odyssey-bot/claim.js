...
async function getToken(privateKey) {
  try {
    const { data } = await axios({
      ...
      params: { wallet: getKeypair(privateKey).publicKey },
      headers: HEADERS,
    });
    ...
    const response = await axios({
      ...
      method: 'POST',
      headers: HEADERS,
      data: { address: publicKey, address_encoded: encodedPublicKey, signature },
    });
...
async function processPrivateKey(privateKey) {
  try {
    ...
    if (profile.wallet_balance > 0) {
      ...
      console.log(`Address ${publicKey}! Details:`.green);
      ...
      console.log(`[ ${moment().format('HH:mm:ss')} ] Please wait to open boxes...`.yellow);
      ...
      if (claimLogin) {
        console.log(`[ ${moment().format('HH:mm:ss')} ] Daily login completed! Status: ${claimLogin.status} | Accumulative Days: ${claimLogin.data.accumulative_days}`.cyan);
      }
      console.log(`[ ${moment().format('HH:mm:ss')} ] All automated tasks completed for ${publicKey}!`.green);
    } else {
...
const getNextRunTime = () => {
  ...
  const nextRun = now.clone().startOf('day').add(10, 'hours');
...
const displayNextRunTime = () => {
  const nextRunTime = getNextRunTime();
  console.log(`Next run: ${nextRunTime.format('DD-MM-YYYY HH:mm:ss')} UTC.`);
};
...
cron.schedule('0 10 * * *', async () => {
  console.log(`[ ${moment().format('DD-MM-YYYY HH:mm:ss')} ] Running scheduled bot task.`);
  await startBot();
}, {
  scheduled: true,
  timezone: "UTC"
